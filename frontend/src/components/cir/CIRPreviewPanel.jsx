...
export default function CIRPreviewPanel({ onPreview, isPending }) {
  ...
  const submit = async (event) => {
    ...
    const response = await onPreview({ username, nas_ip: nasIp })
    setResult(response)
  }
        ...
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm">
          ...
          {result.mapping && (
            <p className="text-emerald-800">
              Winner target: {result.mapping.nas_ip || result.mapping.segment_name || result.mapping.nas_category_name || 'resolved'}
            </p>
          )}
          <ul className="mt-2 text-emerald-800 list-disc list-inside">
            {result.trace?.map((item, index) => (
              <li key={`${item.step}-${index}`}>
                {item.step}: {item.matched ? 'matched' : 'not matched'}
              </li>
            ))}
          </ul>
        </div>
