...
from app.models.models import RadAcct, RadPostAuth, UserNasPrivilegeMap
...
def test_user_nas_privilege_map_unique_constraints_match_init_sql():
    table_constraints = {
        c.name: tuple(c.columns.keys())
        for c in UserNasPrivilegeMap.__table__.constraints
...
def test_user_nas_privilege_map_segment_fk_ondelete_restrict():
    segment_fk = next(
        fk
        for fk in UserNasPrivilegeMap.__table__.c.segment_id.foreign_keys
...
def test_user_nas_privilege_map_nas_category_fk_target_and_ondelete_set_null():
    category_fk = next(iter(UserNasPrivilegeMap.__table__.c.nas_category_id.foreign_keys))
...
def test_user_nas_privilege_map_fk_constraints_have_expected_column_composition():
    fk_constraints = {
        fk.name: tuple(fk.column_keys)
        for fk in UserNasPrivilegeMap.__table__.foreign_key_constraints
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (UserNasPrivilegeMap, "calling_station_id", "AA:BB:CC:DD:EE:FF"),
        (UserNasPrivilegeMap, "calling_station_id", "AA-BB-CC-DD-EE-FF"),
        (UserNasPrivilegeMap, "calling_station_id", "AABB.CCDD.EEFF"),
        (UserNasPrivilegeMap, "calling_station_id", "AABBCCDDEEFF"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (UserNasPrivilegeMap, "calling_station_id"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (UserNasPrivilegeMap, "calling_station_id", "AA::BB::CC::DD::EE::FF"),
        (UserNasPrivilegeMap, "calling_station_id", "A:A:B:B:C:C:D:D:E:E:F:F"),
    ],
)
...
def test_user_nas_privilege_map_constraints_are_present_in_init_sql_text():
    init_sql = (
...
def test_user_nas_privilege_map_fk_ondelete_clauses_present_in_init_sql_text():
    init_sql = (
        Path(__file__).resolve().parents[2] / ".." / "database" / "init.sql"
    ).read_text(encoding="utf-8").lower()

    assert (
        "alter table user_nas_privilege_map add constraint fk_unpm_category "
        "foreign key (nas_category_id) references nas_categories(id) on delete set null"
    ) in init_sql
    assert (
        "alter table user_nas_privilege_map add constraint fk_unpm_segment "
