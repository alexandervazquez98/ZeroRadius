...
class TestMacAssignments:
    async def test_mac_only_assignment_created(
        self, async_client, superadmin_token, base_mac_payload
    ):
        payload = {
            **base_mac_payload,
            "username": "mac_test_user_only",
    ...
    async def test_ip_and_mac_assignment_created(
        self, async_client, superadmin_token, base_mac_payload
    ):
        payload = {
            **base_mac_payload,
    ...
    async def test_mac_and_category_rejected(
        self, async_client, superadmin_token, base_mac_payload
    ):
        payload = {
            **base_mac_payload,
        ...
        assert (
            "exactly one targeting method required" in resp.text.lower()
            or "method required" in resp.text.lower()
        )

    async def test_ip_mac_and_category_rejected(
        self, async_client, superadmin_token, base_mac_payload
    ):
        payload = {
            **base_mac_payload,
    ...
    async def test_invalid_mac_format_rejected(
        self, async_client, superadmin_token, base_mac_payload
    ):
        payload = {**base_mac_payload, "calling_station_id": "invalid-mac-format"}
        resp = await async_client.post(
            "/access-policies/assignments",
            json=payload,
            headers={"Authorization": f"Bearer {superadmin_token}"},
    ...
    async def test_mac_formats_normalized(
        self,
        async_client,
        superadmin_token,
        base_mac_payload,
        ...
        payload = {
            **base_mac_payload,
            "username": f"mac_test_{expected_normalized}_{int(time.time() * 1000)}",
            "calling_station_id": mac_input,
        }
        resp = await async_client.post(
            "/access-policies/assignments",
            json=payload,
