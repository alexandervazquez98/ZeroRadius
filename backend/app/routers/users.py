...
from app.schemas.schemas import RadCheckCreate, RadCheckOut, RadReplyCreate, RadReplyOut, RadCheckUpdate
...
async def create_user_check(
    ...
    await log_audit(db, current_user.username, "CREATE", "radcheck", user.username, new_value=user.model_dump())
    return new_user
...
async def update_user_check(
    ...
    old_data = {"username": user.username, "attribute": user.attribute, "value": user.value}
    ...
    await log_audit(db, current_user.username, "UPDATE", "radcheck", user.username, old_value=old_data, new_value=update_data)
    return user
...
async def delete_user_check(
    ...
    await log_audit(db, current_user.username, "DELETE", "radcheck", user.username, old_value=old_data)
    return {"ok": True}
...
async def create_user_reply(
    ...
    await log_audit(db, current_user.username, "CREATE", "radreply", reply.username, new_value=reply.model_dump())
    return new_reply
...
async def delete_user_reply(
    ...
    old_data = {"username": item.username, "attribute": item.attribute, "value": item.value}
    await db.delete(item)
    await db.commit()
    
    await log_audit(db, current_user.username, "DELETE", "radreply", item.username, old_value=old_data)
    return {"ok": True}
